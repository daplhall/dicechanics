...
class TestLevel2(dice_unittest.TestCase):
	...
	def test_addint(self):
		newpool = self.pool + 1
		self.assertSequenceEqual(newpool._bag, [tts.d6, tts.Die([1])])

	def test_addfloat(self):
		newpool = self.pool + 1.5
		self.assertSequenceEqual(newpool._bag, [tts.d6, tts.Die([1.5])])

	def test_adddice(self):
		newpool = self.pool + tts.d10
		self.assertSequenceEqual(newpool._bag, [tts.d6, tts.d10])
