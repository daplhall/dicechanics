...
def test_max_non_selective(pool_3d6):
	...
	assert d == ds.Die(dict(zip([1, 2, 3, 4, 5, 6], [1, 7, 19, 37, 61, 91])))


def test_min_non_selective(pool_3d6):
	d = pool_3d6.perform(min)
	assert d == ds.Die(
		dict(zip([1, 2, 3, 4, 5, 6], [1, 7, 19, 37, 61, 91][::-1]))
	)
